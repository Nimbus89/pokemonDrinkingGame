...
public class ChampionGaryAftereffectController : AftereffectController
{
    ...
    public override void applyEffect()
    {
        GUIController.Instance.DisplayDialogThenYesNoButtons("Have you finished your drink yet?", (bool answeredYes) => {
            if (answeredYes)
            {
                GUIController.Instance.DisplayBasicModal("You took down his last POKeMON! You are the new POKeMON League Champion!", () =>
                {
                    player.move(1);
                });
            }
            ...
            {
                GUIController.Instance.DisplayBasicModal("You couldn't defeat him this time. Try again next turn!", () =>
                {
                    player.endTurn();
                });
            }
